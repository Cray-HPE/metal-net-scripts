...
net_file_list = ['CAN.yaml', 'HMN.yaml', 'HMNLB.yaml', 'NMNLB.yaml',
                 'NMN.yaml']
...
if len(missing_files) > 0:
    print(f'Missing {", ".join(missing_files)} in directory,'
          f' please verify {", ".join(net_file_list)} are all'
          f' located inside the directory')
    sys.exit()
...
for ips in switch_ips:
    ...
    try:
        response = session.post(base_url + 'login', data=creds, verify=False,
                                timeout=5)
    ...
    if not _response_ok(response, "PUT"):
        logging.warning("FAIL: Login failed with status code %d: %s" % (
    ...
    for p in prefix:
        ...
        prefix_list_entry_url = base_url + 'system/prefix_lists/{0}/prefix_list_entries'.format(
            p)
    ...
    for name in ncn_names:
        route_map_entry_url = base_url + 'system/route_maps/{0}/route_map_entries'.format(
            name)
        route_map['name'] = name
    ...
    if '10.06' in firmware['current_version']:
        for ncn, names in zip(ncn_nmn_ips, ncn_names):
            ...
            bgp_neighbor10_06['route_maps']['ipv4-unicast'][
                'in'] = '/rest/v10.04/system/route_maps/' + names
    ...
    if '10.05' in firmware['current_version']:
        for ncn, names in zip(ncn_nmn_ips, ncn_names):
            ...
            bgp_neighbor_url = base_url + 'system/vrfs/default/bgp_routers/65533/bgp_neighbors'
            bgp_neighbor10_05['route_maps']['ipv4-unicast'][
                'in'] = '/rest/v10.04/system/route_maps/' + names
    ...
    logout = session.post(
        f'https://{ips}/rest/v10.04/logout')  # logout of switch
